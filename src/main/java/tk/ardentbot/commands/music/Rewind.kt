...
class Rewind(commandSettings: CommandSettings) : Command(commandSettings) {
            ...
            if (track != null) {
                ...
                (ownerId, ignoreCase = true)) {
                    try {
                        ...
                        if (position - move < 0) {
                            sendTranslatedMessage("You can't rewind further back than the beginning of the song!", channel, user)
                            return
                        } else {
                            track.track.position = position - move
                            sendTranslatedMessage("I went back ${move / 1000} seconds", channel, user)
                        }
                    } catch(e: Exception) {
                        sendTranslatedMessage("You need to provide a valid integer to rewind", channel, user)
                    }
                } else {
